...
class CustomFlowLayout: UICollectionViewLayout {
    ...
    private let firstImageHeight: CGFloat = 300
    ...
    override func prepare() {
        ...
        for column in 0..<numberOfColumns {
            xOffset.append((CGFloat(column) * columnWidth) + (cellPadding * CGFloat(column)))
        }
        ...
        for item in 0..<collectionView.numberOfItems(inSection: 0) {
            ...
            let frame = CGRect(x: xOffset[column],
                               ...
                               width: columnWidth,
                               height: height)
            let insetFrame = frame.inset(by: UIEdgeInsets(top: 0, left: 0, bottom: cellPadding, right: 0))
            //5
